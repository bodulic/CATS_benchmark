...
#Determining the number and length of introduced insertions
 set.seed(SEED + i)  
 region_N <- sample(1 : REGION_N, size = 1)
 region_length <- round((MUT_FACTOR * length(transcript)) / region_N, 0)
 
#Determining insertion coordinates while excluding already introduced insertions (in case of multiple introductions)
 exclude_range <- c()
 for (j in 1 : region_N) {
  if (j > 1) {
   exclude_range <- c(exclude_range, seq(from = start_sample - region_length + 1, to = start_sample + region_length - 1, by = 1))
  }
  start_sample_range <- 2 : (length(transcript) - 1)
  set.seed(SEED + i + j)  
  start_sample <- sample(start_sample_range[start_sample_range %in% exclude_range == F], size = 1)
  set.seed(SEED + i + j)  
  sequence_to_insert <- DNAString(paste0(sample(c("A", "T", "G", "C"), size = region_length, replace = T), collapse = ""))
  transcript <- DNAString(paste0(transcript[1 : start_sample], sequence_to_insert, transcript[(start_sample + 1) : length(transcript)]))
 }
...
names(tr_to_mutate) <- paste(names(tr_to_mutate), "insertion_mut", sep = "_")
transcriptome <- c(transcriptome, tr_to_mutate)

#Writing mutated transcriptome assembly to file
writeXStringSet(transcriptome, filepath = paste(OUT_PREF, "insertion", MUT_FACTOR, SEED, sep = "_"), format = "fasta", append = F, compress = F, compression_level = NA)