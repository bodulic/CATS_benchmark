...
mut_analysis_transcript_scores <- fread("mutation_analysis_CATS_rf_transcript_scores_for_figure4.tsv")
...
#Plotting Figure 4a (distribution of CATS-rf assembly scores across mutation types and levels) (single-mutation assemblies)
fig_4a <- ggplot(mutated_assembly_scores_single, aes(x = mut_level, y = cats_rf_assembly_score, fill = mut_level, color = mut_level)) +
...
ggsave(fig_4a, file = "Figure_4a.svg", height = 1.6, width = 6.2)
...
#Plotting Figure 4b (distribution of coverage / accuracy / local fidelity component in mutated and native transcripts across mutation types and levels) (single-mutation assemblies)
...
fig_4b <- plot_grid(mut_transcript_score_ins, mut_transcript_score_mis, mut_transcript_score_del, nrow = 1)
ggsave(fig_4b, file = "Figure_4b.svg", height = 1.6, width = 4.7)
...
#Plotting Figure 4c (distribution of CATS-rf coverage component across redundancy levels) (single-mutation assemblies)
fig_4c <- ggplot(red_transcript_cov_comp_N, aes(x = mut_level, y = N, fill = cats_rf_coverage_component_cat)) +
 geom_bar(stat = "identity", position = "fill", width = 0.8, size = 0.3, color = "grey50", alpha = 0.8) +
 theme_minimal() +
 scale_fill_manual(values = c("#E64B35", "#F39B7F", "#B09C85", "#00A087", "#4DBBD5", "#3C5488"), name = expression(italic(S[c])), labels = c("0.8-1", "0.6-0.8", "0.4-0.6", "0.2-0.4", "0-0.2", "0")) +
 theme(legend.position = "right", legend.margin = margin(0, 0, 0, 0)) +
 theme(legend.title = element_text(size = 6.5, face = "italic")) +
...
ggsave(fig_4c, file = "Figure_4c.svg", height = 1.45, width = 2)
...
#Plotting Figure 4d (distribution of mean CATS-rf integrity component per assembly across fragmentation levels) (single-mutation assemblies)
fig_4d <- ggplot(frag_transcripts_int_comp_mean, aes(x = mut_level, y = cats_rf_integrity_component_mean, fill = mut_level, color = mut_level)) +
 geom_boxplot(width = 0.8, size = 0.3, alpha = 0.75, color ="grey40", outlier.shape =  NA) +
...
ggsave(fig_4d, file = "Figure_4d.svg", height = 1.5, width = 1.8)
...
#Plotting Figure 4e (PCA of mean score components per assembly by mutation type (single-mutation assemblies, mutation levels 3 and 4))
fig_4e <- ggbiplot(mutated_assembly_score_components_pca, groups = pca_groups, ellipse = T, ellipse.level = 0.95, ellipse.linewidth = 0.5, ellipse.alpha = 0.1, circle = T, alpha = 0, var.axes = F) +
...
ggsave(fig_4e, file = "Figure_4e.svg", height = 1.5, width = 5)
...
#Plotting Figure 4f (distribution of CATS-rf assembly scores across mutation levels (multiplicative-mutation assemblies)
fig_4f <- ggplot(mutated_assembly_scores_multi, aes(x = mut_level, y = cats_rf_assembly_score, fill = mut_level, color = mut_level)) +
 geom_boxplot(width = 0.8, size = 0.3, alpha = 0.75, color ="grey40", outlier.shape =  NA) +
...
ggsave(fig_4f, file = "Figure_4f.svg", height = 1.5, width = 2.4)