#!/usr/bin/env Rscript
#Script for generating Figure 2 and Figure 3
...
controlled_simulated_transcript_scores[, "coverage" := paste0(coverage, "×")]
...
#Calculating and writing mean transcript F-scores and CATS-rf assembly scores to file (for Figure 7)
...
write.table(controlled_simulated_assem_cats_rf_f_scores, file = "controlled_simulated_assembly_cats_rf_f_scores_for_figure7.tsv", sep = "\t", row.names = F, col.names = T, quote = F)
...
controlled_simulated_transcript_scores[, "coverage" := factor(coverage, levels = c("1–4×", "5–10×", "11–20×", "21–30×", "31–40×", "41–50×", "51–100×"))]
...
fig_2a <- ggplot(data = controlled_simulated_transcript_scores, aes(x = cats_rf_transcript_score, group = assembler, color = assembler)) +
 ...
 xlab(expression(italic(S[t]))) +
 ylab("Relative density") +
 theme(axis.title = element_text(size = 7)) +
 theme(axis.text.x = element_text(size = 6, angle = 45, hjust = 0.9)) +
 theme(axis.text.y = element_text(size = 6.5))
  
ggsave(fig_2a, file = "Figure_2a.svg", height = 2.4, width = 3.7)
...
fig_2b <- ggplot(data = controlled_simulated_transcript_scores, aes(x = cats_rf_transcript_score, group = coverage, color = coverage)) +
 ...
 theme_minimal() +
 scale_color_manual(values = c("#3C5488", "#4DBBD5", "#00A087", "#B09C85", "#F6C85F", "#F46D43", "#E64B35"), name = "Coverage") +
 theme(legend.position = "bottom", legend.justification = "center", legend.margin = margin(0, 0, 0, 0), legend.box.margin = margin(-6, -6, -3, -6)) +
 ...
 theme(strip.text.y = element_text(size = 4.3)) + 
 theme(panel.grid = element_blank()) +
 theme(panel.background = element_rect(fill = NA, color = "grey70")) +
 xlab(expression(italic(S[t]))) +
 ylab("Relative density") +
...
ggsave(fig_2b, file = "Figure_2b.svg", height = 3.75, width = 3.4)
...
fig_2c <- ggcorrplot(pairwise_cats_rf_comp_corr_matrix_median, method = "square", type = "full", ggtheme = ggplot2::theme_minimal(), colors = c("#E64B35", "white", "#4DBBD5"), show.legend = F, lab = T, show.diag = T, lab_size = 2.1, digits = 2, tl.cex = 8, tl.srt = 0) +
 scale_x_discrete(labels = c(expression(italic(S[c])), expression(italic(S[a])), expression(italic(S[l])), expression(italic(S[i])))) + 
 scale_y_discrete(labels = c(expression(italic(S[c])), expression(italic(S[a])), expression(italic(S[l])), expression(italic(S[i])))) +
 theme(axis.text = element_text(face = "italic", size = 7))

ggsave(fig_2c, file = "figure_2c.svg", height = 1.3, width = 1.3)
...
fig_2d <- ggplot(data = metric_f_score_cor_results, aes(x = category, y = cor_coeff, fill = tool)) +
 geom_boxplot(width = 0.8, size = 0.2, alpha = 0.75, color ="grey40", outlier.shape = NA) +
 theme_minimal() +
 ...
 theme(axis.text.x = element_text(size = 5.3, angle = 45, hjust = 0.9)) +
 theme(axis.text.y = element_text(size = 5.8)) 

ggsave(fig_2d, file = "Figure_2d.svg", height = 1.5, width = 2.2)
...
for (i in 1 : length(assemblies)) {
 ...
 controlled_simulated_transcript_scores_subset <- controlled_simulated_transcript_scores[coverage == "1–4×" & assembly == assemblies[i]]
 ...
 cats_rf_score_f_score_corr_list_cov_1[[i]] <- get_score_f_score_corr(assemblies[i], "1–4×")
 ...
 cats_rf_score_f_score_corr_list_cov_2[[i]] <- get_score_f_score_corr(assemblies[i], ">= 5–10×")
...
cats_rf_score_f_score_corr_list <- list(`S. cerevisiae` = rbindlist(cats_rf_score_f_score_corr_list_species_1), `H. sapiens` = rbindlist(cats_rf_score_f_score_corr_list_species_2), `Coverage = 1–4×` = rbindlist(cats_rf_score_f_score_corr_list_cov_1), `Coverage >= 5–10×` = rbindlist(cats_rf_score_f_score_corr_list_cov_2), `ER 0.005` = rbindlist(cats_rf_score_f_score_corr_list_mismatch_1), `ER 0.02` = rbindlist(cats_rf_score_f_score_corr_list_mismatch_2), `rnaSPAdes` = rbindlist(cats_rf_score_f_score_corr_list_assembler_1), `Trinity` = rbindlist(cats_rf_score_f_score_corr_list_assembler_2))
...
f_score_plots <- lapply(names(cats_rf_score_f_score_corr_list), function(subset_category) {
 ...
 ggplot(data = cats_rf_score_f_score_corr_list_subset, aes(x = category, y = cor_coeff, fill = tool)) +
  ...
  theme(axis.text.x = element_text(angle = 40, hjust = 0.9, size = 5)) +
...
fig_2e <- ggplot(data = cor_with_f_score_assembly_median_iqr_long,aes(x = coverage, y = cor_median, color = tool)) +
 geom_point(position = position_dodge(width = 0), shape = 16, size = 0.8, alpha = 0.7) +
 ...
 theme(strip.text.x = element_text(size = 4.3, face = "italic")) +
 theme(strip.text.y = element_text(size = 4.4)) +
 theme(panel.grid = element_blank()) +
 theme(panel.background = element_rect(fill = NA, color = "grey70")) +
 coord_flip(expand = T)  +
 xlab("Coverage") +
...
ggsave(fig_2e, file = "Figure_2e.svg", height = 3.3, width = 4.05)
...
cont_sim_assem_cats_rf_f_score_dotplot <- ggplot(data = controlled_simulated_transcript_score_means, aes(x = transcript_f_score_mean, y = cats_rf_tr_score_mean, color = species)) +
 ...
 theme(axis.title.y = element_text(size = 6)) +
 theme(axis.text.x = element_text(size = 6.5, angle = 45, hjust = 0.9)) +
 theme(axis.text.y = element_text(size = 6.5)) +
 annotate("text", x = 0, y = 0.956, label = paste0("italic(\u03C1) == ", cont_sim_assem_cats_rf_f_score_cor_coeff), parse = T, size = 2.2, hjust = 0) +
 ggtitle("CATS-rf") +
...
cont_sim_assem_rsem_eval_f_score_dotplot <- ggplot(data = controlled_simulated_transcript_score_means, aes(x = transcript_f_score_mean, y = rsem_eval_tr_score_mean, color = species)) +
 ...
 theme(axis.text.x = element_text(size = 6.5, angle = 45, hjust = 0.9)) +
 theme(axis.text.y = element_text(size = 6.5)) +
 annotate("text", x = 0, y = controlled_simulated_transcript_score_means[, max(rsem_eval_tr_score_mean)] - 29500, label = paste0("italic(\u03C1) == ", cont_sim_assem_rsem_eval_f_score_cor_coeff), parse = T, size = 2.2, hjust = 0) +
 ggtitle("RSEM-EVAL") +
 theme(plot.title = element_text(size = 6)) 
...
cont_sim_assem_transrate_f_score_dotplot <- ggplot(data = controlled_simulated_transcript_score_means, aes(x = transcript_f_score_mean, y = transrate_tr_score_mean, color = species)) +
 ...
 theme(axis.text.x = element_text(size = 6.5, angle = 45, hjust = 0.9)) +
 theme(axis.text.y = element_text(size = 6.5)) +
 annotate("text", x = 0, y = 0.96, label = paste0("italic(\u03C1) == ", cont_sim_assem_transrate_f_score_cor_coeff), parse = T, size = 2.2, hjust = 0) +
 ggtitle("TransRate") +
 theme(plot.title = element_text(size = 6)) 

fig_2f <- plot_grid(cont_sim_assem_cats_rf_f_score_dotplot, cont_sim_assem_rsem_eval_f_score_dotplot, cont_sim_assem_transrate_f_score_dotplot, nrow = 2)
ggsave(fig_2f, file = "Figure_2f.svg", height = 2.9, width = 3)

#Importing merged realistic simulations from file
realistically_simulated_transcript_scores <- fread("merged_realistically_simulated_transcript_scores_for_figure3.tsv")
...
realistically_simulated_transcript_scores[, "coverage" := paste0(coverage, "×")]
...
realistically_simulated_transcript_scores[, "coverage" := factor(coverage, levels = c("20×", "40×", "60×", "80×"))]
...
#Plotting Figure 3a (distribution of CATS-rf scores according to species, assembler, and coverage (realistically simulated assemblies)
fig_3a <- ggplot(data = realistically_simulated_transcript_scores, aes(x = cats_rf_transcript_score, group = coverage, color = coverage)) +
 geom_density(aes(y = ..density.. / max(..density..)), adjust = 3, linewidth = 0.25) +
 theme_minimal() +
 scale_color_manual(values = c("#4DBBD5", "#00A087", "#F6C85F", "#E64B35"), name = "Coverage") +
 theme(legend.position = "bottom", legend.justification = "center", legend.margin = margin(0, 0, 0, 0), legend.box.margin = margin(-6, -6, -3, -6)) +
 ...
 theme(strip.text.x = element_text(size = 5.2))  +
 theme(panel.grid = element_blank())  +
 theme(panel.background = element_rect(fill = NA, color = "grey70")) +
 xlab(expression(italic(S[t]))) +
 ylab("Relative density") +
 theme(axis.title.x = element_text(size = 7, face = "italic")) +
 theme(axis.title.y = element_text(size = 6)) +
 theme(axis.text.x = element_text(size = 6.25, angle = 45, hjust = 0.9)) +
 theme(axis.text.y = element_text(size = 6.5) )

ggsave(fig_3a, file = "Figure_3a.svg", height = 5.1, width = 3.5)
...
#Adjusting factor levels for species, coverage, assembler, and tool
cor_with_f_score_assembly_median_iqr_long[, "species" := factor(species, levels = c("S. cerevisiae", "C. elegans", "D. melanogaster", "A. thaliana", "M. musculus", "H. sapiens"))]
cor_with_f_score_assembly_median_iqr_long[, "coverage" := factor(coverage, levels = rev(c("20×", "40×", "60×", "80×")))]
cor_with_f_score_assembly_median_iqr_long[, "assembler" := factor(assembler, levels = c("rnaSPAdes", "Trinity","IDBA-tran","SOAPdenovo-Trans"))]

#Plotting Figure 3b (correlation between CATS-rf, RSEM-EVAL, and TransRate transcript scores with transcript F-scores per species, coverage, and assembler) (realistically simulated assemblies)
fig_3b <- ggplot(data = cor_with_f_score_assembly_median_iqr_long, aes(x = coverage, y = cor_median, color = tool)) +
...
ggsave(fig_3b, file = "Figure_3b.svg", height = 2.5, width = 3.5)
...
#Plotting Figure 3c (correlation between CATS-rf, RSEM-EVAL, and TransRate mean transcript scores with mean transcript F-scores) (realistically simulated assemblies)
...
fig_3c <- plot_grid(real_sim_assem_cats_rf_f_score_dotplot, real_sim_assem_rsem_eval_f_score_dotplot, real_sim_assem_transrate_f_score_dotplot, nrow = 1)
ggsave(fig_3c, file = "Figure_3c.svg", height = 1.5, width = 4.8)

#Importing merged public transcript scores from file
public_transcript_scores <- fread("merged_public_transcript_scores_for_figure3.tsv")
...
#Plotting Figure 3d (correlation between CATS-rf, RSEM-EVAL, and TransRate mean transcript scores with mean transcript F-scores) (public assemblies)
...
fig_3d <- plot_grid(pub_tr_cats_rf_f_score_mean_dotplot, pub_tr_rsem_eval_f_score_mean_dotplot, pub_tr_transrate_f_score_mean_dotplot, nrow = 1)
ggsave(fig_3d, file = "Figure_3d.svg", height = 1.5, width = 4.6)