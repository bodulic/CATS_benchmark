#!/usr/bin/env Rscript
#Script for generating Supplementary Figure 10
...
ref_tr_names <- fread("chimeric_ref_transcriptome_names_for_supp_figure10.tsv")
setnames(ref_tr_names, c("transcript", "transcriptome"))

#Importing CATS-rb structurally inconsistent transcripts
cats_rb_str_inc_tr <- fread("str_inconsistent_transcripts_for_supp_figure10.tsv")
...
#Plotting Supplementary Figure 10 (performance of CATS-rb in classifying chimeric sequences as structurally inconsistent)
supp_fig10 <- ggplot(data = chim_detection_results_longer, aes(x = species, y = metric_value, fill = metric)) +
...
ggsave(supp_fig10, file = "Supplementary_figure_10.tiff", height = 1.1, width = 3.2, dpi = 600, bg = "white")